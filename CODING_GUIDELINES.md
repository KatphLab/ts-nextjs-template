...
- Reuse existing modules and components when possible.
...
```Rhizara/CODING_GUIDELINES.md#L1-1
// BAD
...
import { MyComponent, MyProps } from '@app/components/MyComponent'

// GOOD — inline type qualifier on the type-only specifier
import { MyComponent, type MyProps } from '@app/components/MyComponent'
```
...
| Layer                 | May depend on                                      |
| --------------------- | -------------------------------------------------- |
| `src/shared/**`       | Nothing inside `src/app/`                          |
| `src/components/**`   | `src/shared/**`                                    |
| `src/app/**`          | `src/app/**`, `src/shared/**`, `src/components/**` |
...
- Do not abbreviate identifiers unless the abbreviation is in the approved list: `props`, `prop`, `ref`, `refs`, `err`, `args`, `env`, `db`, `temp`, `fn`, `req`, `res`.
...
- Test any change to: UI copy visible to users, state transitions, side effects (storage or network), loading and error states, and data contract or output shape.
...
- Keep keyboard navigation functional for every UI edit.
- Ensure every interactive control has an accessible name or label.
- Preserve readable contrast and avoid regressions in loading and error feedback.
